...
type NormalizedSpecifierMap = Array<[string, URL | null]>;

type NormalizedScopes = Array<[string, NormalizedSpecifierMap]>;

export function parse(json: any, baseUrl: URL) {
    ...
    if (('imports' in json)) {
        ...
        if (typeof imports !== 'object') {
            throw new TypeError(`The "imports" top-level key needs to be a JSON object.`);
        }
    ...
    if (('scopes' in json)) {
        ...
        if (typeof scopes !== 'object') {
            throw new TypeError(`The "scopes" top-level key needs to be a JSON object.`);
        }
...
function sortAndNormalizeSpecifierMap(originalMap: Record<string, any>, baseUrl: URL) {
    const normalized: NormalizedSpecifierMap = [];
...
function sortAndNormalizeScopes(originalMap: Record<string, any>, baseUrl: URL) {
    const normalized: NormalizedScopes = [];
    for (const scopePrefix in originalMap) {
        ...
        if (typeof potentialSpecifierMap !== 'object') {
            throw TypeError(`The value of the scope with prefix ${scopePrefix} needs to be a JSON object.`);
        }
        const scopePrefixUrl = parseUrl(scopePrefix, baseUrl);
        if (!scopePrefixUrl) {
            console.warn(`The scope prefix URL ${scopePrefixUrl} was not parseable.`);
            continue;
        }
