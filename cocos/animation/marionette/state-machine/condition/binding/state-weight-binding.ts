import { ConditionEvalContext, StateWeightObserver, TransitionBindingContext } from '../condition-base';
...
export class TCStateWeightBinding extends TCBinding<TCBindingValueType.FLOAT> {
    ...
    public bind (_context: ConditionEvalContext, transitionBindingContext: TransitionBindingContext): TCBindingEvaluation<number> | undefined {
        return new TCStateWeightBindingEvaluation(transitionBindingContext.createStateWeightVisitor());
    }
}

class TCStateWeightBindingEvaluation implements TCBindingEvaluation<number> {
    constructor (private _stateWeightObserver: StateWeightObserver) { }

    evaluate (): number {
        return this._stateWeightObserver.observe();
    }
}
