...
@ccclass(`${CLASS_NAME_PREFIX_ANIM}ChoosePoseBase`)
@poseGraphNodeHide()
export abstract class ChoosePoseBase extends PoseNode {
    constructor (initialChoiceCount = 0) {
        super();
        this._poses.length = initialChoiceCount;
        this._poses.fill(null);
        this._alteringDurations.length = initialChoiceCount;
        this._alteringDurations.fill(0.0);
    }
    ...
    public doEvaluate (context: AnimationGraphEvaluationContext) {
        ...
        return ChoosePoseBase.evaluateDefaultPose(context, spaceRequirement);
    }
    ...
    @serializable
    protected _alteringDurations: number[] = [];
...
class EvaluationRecord {
    ...
    public update (deltaTime: number, newChoseIndex: number, alteringDurations: readonly number[]) {
        assertIsTrue(deltaTime >= 0.0);

        this._checkAlternation(newChoseIndex, alteringDurations);
    ...
    private _checkAlternation (newChoseIndex: number, alteringDurations: readonly number[]) {
        const {
            _items: items,
        ...
        // Otherwise, plan so that in new chosen pose's altering duration:
        // - new chosen pose starts transitioning to weight 1,
        // - other poses start transitioning to weight 0.

        const newAlteringDuration = Math.max(alteringDurations[newChoseIndex], 0.0);
        ...
        if (newAlteringDuration < ZERO_ALTERING_DURATION_THRESHOLD) {
            for (let iPose = 0; iPose < nPoses; ++iPose) {
                const item = items[iPose];
        ...
        this._blendingDuration = newAlteringDuration;
    }
}
