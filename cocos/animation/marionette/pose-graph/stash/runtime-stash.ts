...
import { PoseNode, PoseNodeBindingContext, PoseNodeSettleContext, PoseNodeUpdateContext, PoseTransformSpaceRequirement } from '../pose-node';

interface RuntimeStash {
    reenter(): void;

    requestUpdate (context: PoseNodeUpdateContext): void;
...
enum StashRecordState {
    ...
    /**
     * The stash is settled. A `reenter` is required to activate the stash.
     */
    SETTLED,
...
class RuntimeStashRecord implements RuntimeStash {
    ...
    public set (stash: PoseGraphStash, context: PoseNodeBindingContext) {
        assertIsTrue(this._state === StashRecordState.UNINITIALIZED, `The stash has already been set.`);
        const node = instantiatePoseGraph(stash.poseGraph, context.outerContext);
        if (node) {
            node.bind(context);
...
export class RuntimeStashManager implements RuntimeStashView {
    ...
    public setStash (id: string, stash: PoseGraphStash, context: PoseNodeBindingContext) {
