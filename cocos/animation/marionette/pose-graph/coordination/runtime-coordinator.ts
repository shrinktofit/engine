...
import { MotionCoordination } from './motion-coordination';

export class RuntimeCoordinator {
    public register (coordination: MotionCoordination): RuntimeCoordinationRecord {
        const {
            group: groupName,
        } = coordination;
...
class Group {
    ...
    public addMember (): RuntimeCoordinationRecordImpl {
        const record = new RuntimeCoordinationRecordImpl();
    ...
    public coordinate () {
        ...
        // To avoid this, we prefer the leader during previous coordination.
        let leaderIndex = 0;
    ...
    private _lastLeader: RuntimeCoordinationRecordImpl | undefined = undefined;
    private _records: RuntimeCoordinationRecordImpl[] = [];
}

class RuntimeCoordinationRecordImpl implements RuntimeCoordinationRecord {
    ...
    public getCoordinatedEnterTime (): number {
        return this.normalizedTime;
    }
}

export interface RuntimeCoordinationRecord {
    notifyRenter(): void;

    notifyUpdate(deltaTime: number, weight: number): void;

    getCoordinatedEnterTime(): number;
}
