...
export default function physicsMaterialTest(env: PhysicsTestEnv) {
    ...
    describe.only(`Physics material`, () => {
        test(`Clone`, () => {
            const properties = Object.freeze({
                friction: 0.12,
                rollingFriction: 0.34,
                spinningFriction: 0.56,
                frictionCombineMode: PhysicsMaterialCombineMode.MIN,
                restitution: 0.78,
                restitutionCombineMode: PhysicsMaterialCombineMode.AVERAGE,
            } as Partial<PhysicsMaterial>);

            const src = fpx(new PhysicsMaterial(), properties);
            const cloned = src.clone();
            expect(cloned).not.toBe(src);
            expect(cloned.id).not.toBe(src.id);
            expect(cloned).toEqual(expect.objectContaining(properties));
        });
