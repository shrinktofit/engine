...
import { AuxiliaryCurve } from "../../cocos/animation/auxiliary-curve";

test(`Shall be ignored in legacy animation system`, () => {
    const curve = new AuxiliaryCurve();
    curve.name = 'xyz';
    const clip = new AnimationClip();
    clip.duration = 1.0;
    clip.auxiliaryCurves.push(curve);
...
describe(`Behavior in marionette`, () => {
    test(`In clip motion`, () => {
        ...
        const curve = new AuxiliaryCurve();
        curve.name = 'SomeCurve';
        curve.curve.assignSorted([
