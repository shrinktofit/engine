...
import { AnimationGraph, AnimationTransition, EmptyStateTransition, isAnimationTransition, PoseExprState, PoseExprTransition, State, StateMachine, SubStateMachine, Transition } from "../../../../cocos/animation/marionette/animation-graph";
    ...
    for (const layerParams of params.layers) {
        ...
        if (layerParams.stashes) {
            for (const [stashId, stashParams] of Object.entries(layerParams.stashes)) {
                ...
                fillPoseExprGraph(stash.graph, stashParams.graph);
            }
        }
...
function fillStateMachine(stateMachine: StateMachine, params: StateMachineParams) {
    const states = Object.entries(params.states ?? {}).reduce((result, [stateId, stateParams]) => {
        ...
        switch (stateParams.type) {
            case 'motion':
                ...
                break;
            case 'pose-expr':
                state = stateMachine.addPoseExprState();
                fillPoseExprGraph((state as PoseExprState).poseExprGraph, stateParams.graph);
                break;
            case 'empty':
...
function fillTransition(transition: Transition, params: TransitionAttributes) {
    ...
    function assertsIsDurableTransition(transition: Transition): asserts transition is (AnimationTransition | EmptyStateTransition | PoseExprTransition) {
        if (!isAnimationTransition(transition) && !(transition instanceof EmptyStateTransition) && !(transition instanceof PoseExprTransition)) {
            throw new Error(`The transition should be animation/empty/pose-expr transition.`);
        }
    }
...
type VariableDeclarationParams = {
    type: 'int',
    value?: number;
} | {
    type: 'float',
