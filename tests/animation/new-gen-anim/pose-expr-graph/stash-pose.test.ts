...
describe(`Stash update`, () => {
    test(`Two nodes request different delta time stash update`, () => {
        ...
        for (const timePoint of [1.15, 1.2]) {
            ...
            expect(poseNodeMock.selfEvaluate_).toBeCalledTimes(1);
            poseNodeMock.selfEvaluate_.mockClear();
        }
    });
});
...
describe(`Dependent stashes`, () => {
    test(`Dependent stashes`, () => {
        const poseNodeMockA = new PoseNodeMock();
        ...
        const animationGraph = createAnimationGraph({
            ...
            layers: [{
                stashes: {
                    ...
                    'StashB': { graph: { rootNode: poseNodeMockA } },
                },
                stateMachine: {
                    states: {
        ...
        // Enter <UseStashA>.
        evalMock.step(0.1);
        expect(poseNodeMockA.reenter_).toBeCalledTimes(1);
        poseNodeMockA.reenter_.mockClear();
        expect(poseNodeMockA.update_).toBeCalledTimes(1);
        expect(poseNodeMockA.update_).toBeCalledWith(expect.objectContaining({
            deltaTime: 0.1,
        }));
        poseNodeMockA.update_.mockClear();
        expect(poseNodeMockA.selfEvaluate_).toBeCalledTimes(1);
    });
