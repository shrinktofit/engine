import { AddPose } from "../../../../cocos/animation/marionette/pose-expressions/add-pose";
import { createAdditivityCheckMock, createPoseExprBindContextMock_WithAdditive, invokePoseExprBindMethod } from "./utils/additive";
...
test(`Additivity inheritance`, () => {
    ...
    function t(initialAdditive: boolean) {
        ...
        const addPose = new AddPose();
        addPose.base = new base_AdditivityCheckMock.PoseExprMock();
        addPose.addition = new addition_AdditivityCheckMock.PoseExprMock();
    
        const context = createPoseExprBindContextMock_WithAdditive(initialAdditive);
    
        invokePoseExprBindMethod(addPose, context);
