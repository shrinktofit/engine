...
test(`Coordination`, () => {
    const WITH_COORDINATION: boolean = true;
    ...
    const graph = createAnimationGraph({
        ...
        layers: [{
            stashes: {
                'f': { graph: { rootNode: { type: 'motion', motion: fixture.motion_f.createMotion(observer.getCreateMotionContext()), coordination: WITH_COORDINATION ? { group: 'locomotion' } : undefined } } },
                'r': { graph: { rootNode: { type: 'motion', motion: fixture.motion_r.createMotion(observer.getCreateMotionContext()), coordination: WITH_COORDINATION ? { group: 'locomotion' } : undefined } } }
            },
            stateMachine: {
                states: {
                    'f': { type: 'pose-expr', graph: { rootNode: { type: 'use-stash', stashId: 'f' } } },
                    'fr': {
                        type: 'pose-expr',
                        graph: {
                            rootNode: {
